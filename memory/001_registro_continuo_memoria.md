...
## Apêndice R9 — Import de contatos por JSON tolerante e contato manual no wizard (2026-09-25)

### 1. Entregue
- **`backend/internal/api/import_json.go` (novo, 332 linhas):** endpoint `POST /contacts/import-json` (router.go). Aceita o **JSON bruto** da lista (até 20MB) e achata a raiz em registros: array no topo, envelope conhecido (`connections`, `contacts`, `data`, `leads`, `decisores`, `lista`…) ou o maior array de qualquer chave, ou objeto único. Aliases pt-BR/en por campo com chaves normalizadas (`LinkedIn URL` → `linkedin_url`), e varredura recursiva (`findLinkedInDeep`) para links dentro de prosa/aninhamento. Registro sem URL de perfil válida é contado como `skipped` (nunca insere sem link — `linkedin_url` é NOT NULL e chave de dedup).
- **`canonicalLinkedInURL`:** normaliza a URL para dedup entre arquivos com grafias diferentes da mesma pessoa — subdomínio regional (`br.`), percent-encoding (`%C3%A9`), query (`?trk=`), barra final e ausência de esquema viram `https://www.linkedin.com/in/<slug>`. **`nameFromLinkedInSlug`:** contato só com link deriva nome legível do slug, senão `{{first_name}}` trava a cadência com `missing_variables`.
- **`insertContactsUpsert` (compartilhado):** upsert em lote via `pgx.Batch` (chunk 500) com `ON CONFLICT (organization_id, linkedin_url) DO UPDATE`; reimportar arquivo sobreposto é atualização, não duplicação. Comentário fixa o motivo: **COPY FROM não é suportado com RLS ativa (SQLSTATE 0A000)**. `HandleSyncLinkedInContacts` (extensão) passou a usar o mesmo helper e a mesma canonicalização — antes tinha o INSERT inline duplicado.
- **`HandleCreateContact`:** canonicaliza a URL e deriva o nome do slug quando vêm vazios; o INSERT já era `ON CONFLICT … DO UPDATE … RETURNING id` (status `upserted`), então a colisão nova (URL equivalente) resolve como update, não como erro. O registro original de cada linha importada fica em `metadata` JSONB (`origem: import-json`), então campos que não viraram coluna (score, evidências) seguem consultáveis.
- **Frontend (`campaigns/new/page.tsx`):** upload de **vários arquivos** de uma vez (loop sequencial, total + detalhe por arquivo, break no primeiro erro) e formulário de **contato manual** (só a URL do LinkedIn é obrigatória; valida `linkedin.com/(in|pub)/` no cliente). `packages/api-client`: `.json` deixa de ir para `sync-linkedin` e passa a ir bruto para `/contacts/import-json`; tipo `ImportContactsFileResult` (imported/skipped/duplicates + inserted/synced por compatibilidade).
- **Fixtures `scripts/list/samples/`:** `bagunca-ptbr.csv` e `bagunca-links.json` — já referenciados pelo `scripts/list/README.md` (pior caso: `;`, aspas, `Sobrenome, Nome`, duplicata, URL de Twitter, registro sem URL).

### 2. Achados do review (não bloqueantes, sem correção nesta rodada)
- **Regex de perfil não ancorada na borda do host:** `linkedin\.com/(?:in/…|pub/…)` casa em qualquer posição do texto, então `notlinkedin.com/in/x` e `evil-linkedin.com/in/x` são aceitos e reescritos para `https://www.linkedin.com/in/x`. Impacto baixo (o valor só alimenta dedup e a visita do worker a `www.linkedin.com`), mas contraria o comentário do próprio arquivo ("valida que a URL é realmente de perfil do LinkedIn"). Correção sugerida: âncora de borda + grupo de captura (RE2 não tem lookbehind) — `(?:^|[^a-z0-9-])((?:[a-z0-9-]+\.)*linkedin\.com/…)`.
- **`api/openapi.yaml` segue sem os endpoints de import** — nem o `/contacts/import` (CSV) pré-existente, nem o `/contacts/import-json` novo. Divergência pré-existente (não introduzida aqui); decidir se import entra no contrato antes de escrever a spec à mão.
- **Sem PRD** para esta feature em `docs/prd/` (Regra 09 exige PRD **antes** de codar; a implementação veio da sessão anterior).

### 3. Verificação (DoD) — medida nesta sessão, na máquina local
- Backend: `go build ./...` exit 0; `go vet ./...` exit 0; `go test -count=1 ./internal/api/...` **ok 0.519s**; `./internal/tests/...` **ok 0.323s** (inclui a suíte cross-tenant RLS).
- Frontend: `npx tsc --noEmit` sem erros; `npx vitest run` **24/24** (5 arquivos).
- Higiene de repo no mesmo push: `frontend/tsconfig.tsbuildinfo` **destrackeado** (o `.gitignore` já dizia "nunca commitar" e o arquivo estava rastreado desde antes da regra) e `graphify-out/` ignorado (2.2MB de cache AST gerado).
- Jev: orchestration (`commit-push-linkedinexus-escopo`, sem escalate; `scope_size` ~2 = concerns separáveis → commits separados de higiene e feature), definition_of_done (`commit-push-linkedinexus-dod`, sem escalate; `completion_readiness` 0.99 = parcialmente pronto com itens de DoD abertos → memória e review feitos antes do push), code_review (`import-json-code-review`, sem escalate; `exposes_secrets` 0.04 e `has_test_coverage` 0.28). Sem `escalate` em nenhuma chamada.
